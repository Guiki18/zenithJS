	...
	async execute(client, message, args) {
		try {
			...
			if (!ownerUsers.includes(message.author.id)) {
				return console.warn(colors.red(`[${message.author.tag}] tried to use the unload command.`));
			}
			...
			try {
				...
				message.react('✅');
				console.log(colors.green(`[${message.author.tag}] unloaded the command \`${command.name}\`.`));
			}
			catch (error) {
				message.react('❌');
				...
				const reloadError = new MessageEmbed()
					.setColor('RED')
					.setTitle(`❌ | There was an error while unloading the command \`${command.name}\` !`)
					.setDescription(`\`\`\`\n${error}\`\`\``)
					.setTimestamp()
					.setFooter({ text: client.user.username, iconURL: client.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }) });
